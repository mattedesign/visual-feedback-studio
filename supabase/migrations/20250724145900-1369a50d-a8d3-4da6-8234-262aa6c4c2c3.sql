...
-- Allow public read access to storage buckets for anonymous users
INSERT INTO storage.objects (bucket_id, name, owner, metadata) VALUES ('analysis-images', '.keep', null, '{}') ON CONFLICT DO NOTHING;
...
CREATE POLICY "Allow anonymous analysis uploads" ON storage.objects
  FOR INSERT
  WITH CHECK (bucket_id = 'analysis-images' AND (auth.uid()::text IS NULL OR auth.uid()::text = (storage.foldername(name))[1]));

CREATE POLICY "Allow anonymous analysis downloads" ON storage.objects  
  FOR SELECT
  USING (bucket_id = 'analysis-images');

CREATE POLICY "Allow anonymous analysis updates" ON storage.objects
  FOR UPDATE
  USING (bucket_id = 'analysis-images' AND (auth.uid()::text IS NULL OR auth.uid()::text = (storage.foldername(name))[1]));

CREATE POLICY "Allow anonymous analysis deletes" ON storage.objects
  FOR DELETE
  USING (bucket_id = 'analysis-images' AND (auth.uid()::text IS NULL OR auth.uid()::text = (storage.foldername(name))[1]));