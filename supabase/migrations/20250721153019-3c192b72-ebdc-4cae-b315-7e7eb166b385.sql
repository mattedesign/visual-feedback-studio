...
-- Add enhanced metadata tracking columns to figmant tables

-- Add pattern violation tracking to figmant_analysis_results if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_results' 
        AND column_name = 'pattern_violations'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_results' 
        AND column_name = 'confidence_metadata'
    ) THEN
        ALTER TABLE figmant_analysis_results 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_results' 
        AND column_name = 'screen_type_detected'
    ) THEN
        ALTER TABLE figmant_analysis_results 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_results' 
        AND column_name = 'enhanced_business_metrics'
    ) THEN
        ALTER TABLE figmant_analysis_results 
...
-- Add screen detection metadata to figmant_analysis_sessions if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_sessions' 
        AND column_name = 'screen_detection_metadata'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'figmant_analysis_sessions' 
        AND column_name = 'pattern_tracking_enabled'
    ) THEN
        ALTER TABLE figmant_analysis_sessions 
