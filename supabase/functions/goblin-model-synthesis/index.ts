...
class AnnotationGenerator {
  generate(recommendations: any[], persona: string, imageCount: number = 1): Annotation[] {
    ...
    // CRITICAL FIX: Don't generate generic annotations if we have Claude's detailed issues
    if (recommendations.length > 0) {
      const hasDetailedIssues = recommendations.some((rec: any) => rec?.suggested_fix || rec?.description);
      
      if (hasDetailedIssues) {
        console.log('✅ Claude detailed issues detected - annotations will be generated by frontend from issues array');
        // Return empty array - frontend will handle annotation creation from persona_feedback.issues
        return [];
      }
    }
