...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Now seed contextual solutions
    const contextualSolutions: ContextualSolution[] = [
      {
        title: "No Credit Card Trial with Progressive Qualification",
        problem_statement_ids: [insertedProblems?.find(p => p.category === 'conversion_decline')?.id || ''],
      ...
      },
      {
        ...
        problem_statement_ids: [insertedProblems?.find(p => p.category === 'competitive_pressure')?.id || ''],
      ...
      },
      {
        ...
        problem_statement_ids: [insertedProblems?.find(p => p.category === 'user_confusion')?.id || ''],
      ...
      },
      {
        ...
        problem_statement_ids: [insertedProblems?.find(p => p.category === 'technical_constraints')?.id || ''],
      ...
      },
      {
        ...
        problem_statement_ids: [insertedProblems?.find(p => p.category === 'stakeholder_demands')?.id || ''],
    ...
    // Now seed some competitor patterns
    const competitorPatterns = [
      {
        pattern_name: "Progressive CTA Optimization",
        description: "Multi-step approach to CTA testing and optimization based on user behavior analysis",
        industry: "SaaS",
        pattern_type: "conversion_optimization",
        examples: {
          implementations: [
            "A/B testing different CTA copy variations",
            "Progressive profiling for lead qualification",
            "Dynamic CTA personalization based on user segment"
          ],
          results: [
            "30-50% improvement in conversion rates",
            "Reduced form abandonment by 25%",
      ...
      },
      {
        ...
        description: "Strategy for responding to competitive threats by building superior versions of competitor features",
        industry: "Technology",
        ...
        examples: {
          implementations: [
            ...
            "Strategic feature bundling for differentiation"
          ],
          results: [
      ...
      {
        pattern_name: "Checkout Flow Optimization",
        description: "Systematic approach to reducing checkout abandonment through UX improvements",
        industry: "E-commerce",
        pattern_type: "conversion_optimization",
        examples: {
          implementations: [
    ...
    if (patternError) {
      throw new Error(`Error inserting competitor patterns: ${patternError.message}`);
    }

    console.log(`✅ Inserted ${insertedPatterns?.length} competitor patterns`);

    const results = {
      success: true,
      message: 'Database seeding completed successfully',
      data: {
        problemStatements: insertedProblems?.length || 0,
        contextualSolutions: insertedSolutions?.length || 0,
        competitorPatterns: insertedPatterns?.length || 0
      }
    };
