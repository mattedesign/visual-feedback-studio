...
Generated by Figmant UX Analysis Platform`;
  ...
  return (
    <div className="visual-analysis-module flex flex-col lg:flex-row h-screen bg-white dark:bg-slate-900">
          ...
          <div className="relative inline-block max-w-full">
            ...
            {annotationsWithPosition.map((annotation, index) => (
              <div
                ...
                title={`${(annotation?.feedback || annotation?.title || 'Annotation').substring(0, 50) + '...'}${annotation?.correctionApplied ? ' (Auto-corrected)' : ''}`}
              >
                {index + 1}
      ...
      {/* Right Panel - Enhanced Annotation Details and Controls */}
            ...
            <div className="space-y-4">
              <div className="bg-white dark:bg-slate-700 p-4 rounded-lg border border-gray-200 dark:border-slate-600">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {selectedAnnotation?.correctionApplied && (
                ...
                {/* NEW: Coordinate validation info */}
                {selectedAnnotation?.correctionApplied && (
                  <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded border border-yellow-200 dark:border-yellow-800">
                    <h6 className="text-xs font-medium text-yellow-800 dark:text-yellow-200 mb-1">
                      Coordinate Correction Applied
                    </h6>
                    <p className="text-xs text-yellow-700 dark:text-yellow-300">
                      {selectedAnnotation?.correctionReasoning || 'Coordinates were adjusted for better accuracy'}
                    </p>
                    {selectedAnnotation?.originalCoordinates && (
                      <div className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
                        Original: ({selectedAnnotation.originalCoordinates.x}%, {selectedAnnotation.originalCoordinates.y}%) → 
                        Corrected: ({selectedAnnotation.x}%, {selectedAnnotation.y}%)
                      </div>
                    )}
                  </div>
                )}
