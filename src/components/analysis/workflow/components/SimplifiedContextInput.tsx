...
export const SimplifiedContextInput = ({
  ...
  const quickSuggestions = [{
    label: "Surprise me",
    icon: "🎲",
    value: "Comprehensive UX analysis with surprising insights and hidden opportunities"
  }, {
    label: "Visual Hierarchy",
    value: "Visual hierarchy and layout analysis - improve information prioritization and scanning patterns"
  }, {
    label: "Comparison",
    value: uploadedImageCount > 1 ? "Comparative analysis across designs for consistency and best practices" : "Single design analysis with industry comparison insights"
  }, {
    label: "Usability",
    value: "Usability and user experience audit - identify friction points and improvement opportunities"
  }, {
    label: "Comprehensive",
    value: "Complete professional UX audit covering accessibility, conversion, visual design, and user experience"
  }, {
    label: "Character",
    value: "Brand personality and visual character analysis - ensure design reflects intended brand voice"
  }];

  // Advanced context templates (hidden by default)
  const advancedTemplates = {
    'E-commerce': ['E-commerce checkout optimization - analyze conversion flow and reduce cart abandonment', 'Product page conversion analysis - optimize product display and purchase journey'],
  ...
  return <div className="space-y-4">
      ...
      transform: showSuggestions ? 'translateY(-20px)' : 'translateY(0)'
        ...
        {showSuggestions && <div className="w-full order-1">
          ...
          borderBottomRightRadius: '24px'
        }} className="flex gap-2 overflow-x-auto px-[16px] py-[12px]">
              {quickSuggestions.map((suggestion, index) => <button key={index} onClick={() => handleSuggestionClick(suggestion)} className={`inline-flex items-center gap-1.5 text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0 ${index === 0 ? '' : 'text-gray-700 hover:text-gray-900'}`} style={index === 0 ? {
            ...
            <div className="mt-3 pt-2 border-t border-gray-100 dark:border-slate-700 hidden">
              <Button variant="ghost" size="sm" onClick={() => setShowAdvanced(!showAdvanced)} className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                {showAdvanced ? <>
            ...
            {showAdvanced && <div className="space-y-3 pt-2">
                {Object.entries(advancedTemplates).map(([category, templates]) => <div key={category} className="space-y-2">
                    <div className="text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                      {category}
                    </div>
                    <div className="space-y-1">
                      {templates.map((template, index) => <button key={index} onClick={() => handleAdvancedTemplate(template)} className="text-left text-xs text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-slate-700 p-2 rounded w-full transition-colors">
                          {template.split(' - ')[0]}
        ...
        <div className="flex items-center justify-between w-full order-2" style={{
        ...
        paddingRight: showSuggestions ? '20px' : '0'
          ...
          gap: '16px',
          borderRadius: '10px',
          background: showSuggestions ? 'linear-gradient(180deg, var(--Shade-7-100, #323232) 0%, var(--Shade-8-100, #222) 100%)' : 'transparent',
          boxShadow: showSuggestions ? '0px 0.5px 1px 0px rgba(255, 255, 255, 0.15) inset, 0px 2px 4px -1px rgba(13, 13, 13, 0.50), 0px -1px 1.2px 0.35px #121212 inset, 0px 0px 0px 1px #333' : 'none',
          ...
          <Button onClick={onAnalyze} disabled={!canAnalyze} className="text-white font-medium disabled:opacity-50" style={{
          display: 'flex',
          padding: '11px 16px',
          justifyContent: 'center',
          alignItems: 'center',
