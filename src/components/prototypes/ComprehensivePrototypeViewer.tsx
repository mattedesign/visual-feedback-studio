...
`;
  ...
  const createIframeContent = (code: { html: string; css: string; js?: string }) => {
    // Check if the HTML contains React/JSX syntax
    const isReactComponent = code.html.includes('className=') || 
                            code.html.includes('{') || 
                            code.html.includes('onClick') ||
                            code.html.includes('useState');
      ...
      return `
        ...
        <body>
          ...
          <script type="text/babel">
            ...
            // Wrap the component code in a function if it's not already
            ${code.html.startsWith('function') ? code.html : `
            ...
            root.render(React.createElement(${code.html.startsWith('function') ? code.html.match(/function\s+(\w+)/)?.[1] || 'PrototypeComponent' : 'PrototypeComponent'}));
          </script>
        </body>
        </html>
      `;
    } else {
      // Handle plain HTML/CSS
      return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          ...
          <style>
            * {
            ...
            /* User styles - safely injected without transformation */
            ${code.css}
          </style>
        </head>
        <body>
          ...
          >
            ...
            <div className="overflow-y-auto" style={{ height: 'calc(95vh - 180px)' }}>
              <div className="p-6">
                ...
                {activeTab === 'after' && 
                  <PrototypeErrorBoundary>
                    {renderCodePreview(prototype.improvement.afterCode, 'After')}
                  </PrototypeErrorBoundary>}
                {activeTab === 'interactive' && 
                  <PrototypeErrorBoundary>
                    {renderCodePreview(prototype.improvement.interactiveDemo, 'Interactive')}
                  </PrototypeErrorBoundary>}
                {activeTab === 'mobile' && 
                  <PrototypeErrorBoundary>
                    {renderCodePreview(prototype.improvement.mobileResponsive, 'Mobile')}
                  </PrototypeErrorBoundary>}
